...
public class OldAccount {
	...
	public static ArrayList<OldAccount> accounts = new ArrayList<OldAccount>();
	
	public OldAccount(String accountName, ArrayList<String> strings) {
		this.accountName = accountName;
		permission = strings;
	}
	
	public OldAccount(String accountName, String[] strings) {
		this.accountName = accountName;
	...
	public static void updateAccounts() {
		...
		for (int i = 0; i < App.d.StringSELECT("SELECT * FROM jada.accounts", "accountName").size(); i++) {
			...
			accounts.add(new OldAccount(name, perm));
		}
	}

	public static OldAccount getSpecificAccount(String accountName) {
		OldAccount wanted = null;
	...
	public static boolean hasPermission(Socket s, Command cmd, OldAccount acc) throws IOException {
	...
	public static boolean canExecute(String string, OldAccount acc) {
		for (int i = 0; i < acc.getPermission().size(); i++) {
